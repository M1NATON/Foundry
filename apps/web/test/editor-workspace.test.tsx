...
import type { Research, Scene, Script } from "@foundry/shared-types";
...
import { researchKeys } from "../src/lib/queries/research";
...
const research: Research = {
  ...
  sources: [
    {
      title: "Kessler syndrome primer",
      url: "https://example.org/kessler",
      note: "Debris cascade numbers",
    },
  ],
  notes: "Orbit altitudes to double-check.",
};
...
function seed(voiceText: string) {
  ...
  qc.setQueryData(researchKeys.detail(PROJECT_ID), research);
  return qc;
}
...
describe("EditorWorkspace", () => {
  ...
  it("shows sources and notes on Research without the canvas or the script", () => {
    const markup = render(seed("Original voiceover"), "research");

    expect(markup).toContain(research.sources[0].title);
    expect(markup).toContain(research.sources[0].url);
    expect(markup).toContain(research.sources[0].note);
    expect(markup).toContain(research.notes);
    // Research — такой же взаимоисключающий вид: ни Producing, ни Script
    // не должны просвечивать сквозь него.
    expect(markup).not.toContain(SCENE_TITLE);
    expect(markup).not.toContain("Original voiceover");
    expect(markup).not.toContain(SCRIPT_PLACEHOLDER);
    expect(markup).not.toContain(script.content);
  });
