...
(md/txt/json/csv/srt/prompts), но и в таймлайн для монтажки — FCPXML и EDL.
Для DaVinci Resolve есть **Resolve pack**: ZIP с FCPXML, SRT-субтитрами и
копией всех медиафайлов под читаемыми именами — для переноса проекта на
другую машину.
...
8. Проект экспортируется в Markdown, text, JSON, CSV, SRT, prompts, а также в FCPXML, EDL и Resolve pack для монтажки.
...
FCPXML собирается с переходами Cross Dissolve на стыках сцен и маркерами
с подсказкой движения (из videoPrompt) прямо на клипах. Формат кадра
таймлайна задаётся полем `frameFormat` проекта: `LANDSCAPE` (1920x1080)
по умолчанию, `PORTRAIT` (1080x1920) для шортсов — меняется через
`PATCH /projects/:id`.

`resolve-pack` отдаёт ZIP: `project.fcpxml`, `subtitles.srt` и папку
`media/` с копиями всех файлов под читаемыми именами (`scene-01-hook.png`,
`scene-01-hook-voice.wav`). FCPXML ссылается на исходные абсолютные пути —
Resolve не разворачивает относительные, поэтому на машине с Foundry импорт
работает сразу. Папка `media/` нужна для переноса: на другом компьютере
хватит одной перелинковки на неё (Медиатека → выделить клипы → «Перелинковать
выбранные медиа»), имена файлов уникальные.
...
| `POST` | `/projects/:id/export` | Экспортировать в `md`, `txt`, `json`, `csv`, `srt`, `prompts`, `fcpxml`, `edl` или `resolve-pack` |
