...
Для DaVinci Resolve есть самодостаточный **Resolve pack**: ZIP с FCPXML,
всеми медиафайлами под читаемыми именами и SRT-субтитрами — распаковал и
импортировал, без ручной перелинковки.
...
`resolve-pack` отдаёт ZIP: `project.fcpxml`, `subtitles.srt` и папка
`media/` со всеми файлами под читаемыми именами (`scene-01-hook.png`,
`scene-01-hook-voice.wav`). Пути в FCPXML относительные
(`file://./media/...`): архив можно распаковать куда угодно. Если монтажка
не подхватит относительные пути, хватит одной перелинковки на папку
`media/` — имена файлов уникальные.
