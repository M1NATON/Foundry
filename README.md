...
**Research -> Script -> Storyboard -> Producing -> Export**

Текущая версия — MVP, ориентированный на полный локальный сценарий работы.
Приложение запускается без внешних API-ключей: для разбивки сценария и
написания скрипта по теме есть детерминированный фолбэк, генерация ассетов
возвращает локальные SVG-заглушки, а BullMQ при этом обрабатывает настоящие
асинхронные задания и статусы. Проект экспортируется не только в документы
(md/txt/json/csv/srt/prompts), но и в таймлайны для монтажек — FCPXML
(DaVinci Resolve, Final Cut), Premiere XML / xmeml (Premiere Pro импортирует
только старый FCP 7 XML) и EDL. Есть и самодостаточный **Resolve pack**: ZIP
с обоими таймлайнами, SRT-субтитрами и копией всех медиафайлов под читаемыми
именами — для переноса проекта на другую машину.
...
1. Проект содержит опциональные записи research и script, а также базовый музыкальный трек.
2. При сохранении сценария сервер рассчитывает количество слов и длительность (с учётом пауз на знаках препинания).
3. Сценарий можно разбить локально или через Gemini, написать с нуля по теме через импорт-промпт либо импортировать готовую JSON-раскадровку.
4. Упорядоченные сцены принадлежат проекту и содержат сгенерированные или загруженные ассеты; у каждого типа выбирается активный вариант.
5. Длительность загруженных аудио/видео меряется ffprobe; длительность сцены берётся из выбранного голосового файла.
6. BullMQ обрабатывает задания ассетов и обновляет статусы ассета и сцены.
7. Панель готовности перечисляет пробелы до экспорта и умеет догенерировать их пакетно.
8. Проект экспортируется в Markdown, text, JSON, CSV, SRT, prompts, а также в FCPXML, Premiere XML, EDL и Resolve pack для монтажки.
...
Все таймлайны собираются из одного списка `TimelineClip`. При экспорте медиа
копируется в `apps/api/exports/<проект>/media/` под читаемыми именами
(`scene-01-hook.png`, `scene-01-hook-voice.wav`), и таймлайны ссылаются на
эти копии по абсолютным путям — импорт работает сразу, а папку
`exports/<проект>/` можно унести на другую машину целиком. FCPXML получает
переходы Cross Dissolve на стыках сцен; оба формата получают маркеры с
подсказкой движения (из videoPrompt) прямо на клипах. Формат кадра задаётся
полем `frameFormat` проекта: `LANDSCAPE` (1920x1080) по умолчанию,
`PORTRAIT` (1080x1920) для шортсов — меняется через `PATCH /projects/:id`.

- **FCPXML** (`fcpxml`) — DaVinci Resolve, Final Cut. Пути нормализуются
  (Windows-слеши, пробелы), иначе Resolve помечает клипы офлайн.
- **Premiere XML** (`premiere-xml`) — FCP 7 XML (xmeml v4). Premiere Pro НЕ
  импортирует FCPXML — только этот старый формат, причём pathurl нужен
  старомодный: `file://localhost/C%3A/...` — современный `file:///C:/...`
  Premiere раскодирует в битый путь и роняет клипы в офлайн. Переходы в
  xmeml не кладём: надёжнее поставить default transition руками.
- **Resolve pack** (`resolve-pack`) — ZIP с содержимым папки экспорта:
  `project.fcpxml`, `project-premiere.xml`, `subtitles.srt` и `media/`.
  На другом компьютере после распаковки хватает одной перелинковки на
  папку `media/` — имена файлов уникальные.
...
| `GET`, `PATCH`, `DELETE` | `/projects/:id` | Получить, изменить (включая `brief` и `frameFormat`) или удалить проект |
| `GET`, `PUT` | `/projects/:id/research` | Получить или сохранить research (источники и заметки) |
...
| `POST` | `/scenes/:id/prompts` | Переписать промпты из voiceover |
| `POST` | `/scenes/:id/duplicate` | Дублировать сцену |
| `PATCH` | `/scenes/:id` | Изменить сцену |
| `PATCH` | `/scenes/:id/active-asset` | Выбрать активный ассет типа (или снять) |
...
| `GET`, `POST`, `PATCH` | `/projects/:id/music` | Получить, сгенерировать/загрузить или выбрать базовый трек |
| `POST` | `/projects/:id/music/upload` | Загрузить готовый базовый трек |
| `POST` | `/projects/:id/assets/generate-missing` | Пакетно догенерировать недостающие ассеты |
| `POST` | `/projects/:id/export` | Экспортировать в `md`, `txt`, `json`, `csv`, `srt`, `prompts`, `fcpxml`, `premiere-xml`, `edl` или `resolve-pack` |
