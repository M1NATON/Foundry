...
Все таймлайны собираются из одного списка `TimelineClip`. FCPXML получает
...
- **Premiere XML** (`premiere-xml`) — FCP 7 XML (xmeml v4). Premiere Pro НЕ
  импортирует FCPXML — только этот старый формат, поэтому для него генератор
  отдельный. Переходы в xmeml не кладём: надёжнее поставить default
  transition руками.
- **Resolve pack** (`resolve-pack`) — ZIP: `project.fcpxml`,
  `project-premiere.xml`, `subtitles.srt` и папка `media/` с копиями файлов
  под читаемыми именами (`scene-01-hook.png`, `scene-01-hook-voice.wav`).
  Таймлайны ссылаются на исходные абсолютные пути — ни Resolve, ни Premiere
  не разворачивают относительные, поэтому на машине с Foundry импорт
  работает сразу. Папка `media/` нужна для переноса: на другом компьютере
  хватит одной перелинковки на неё, имена файлов уникальные.
